...
| 3 | [Verify `beta` on Render is actually live](story-04-e2e-integration-verification/task-03-verify-render-beta-live.md) | GENERATE-DOCUMENT | IN PROGRESS | Confirmed (via Render CLI) that `grade-ops-ai-api`/`grade-ops-ai-agents` Render services exist, have a deploy history, and auto-deploy is wired to this repo — or a documented finding that they don't, same spirit as `009`'s GCP finding |
...
| 3 | The `gradeops-agents` Render service (`beta`) tracks branch `master` instead of `develop`. `gradeops-api` correctly tracks `develop` and is current, but `master`'s last commit touching `agents/` is a month older than `develop`'s (38 commits behind, missing the entire `002-groq-genai-provider` story). Auto-deploy-on-push is genuinely configured and firing — it is just deploying the wrong branch, so the live `beta` `agents/` service does not reflect current code. | Render service `gradeops-agents` (`srv-d8oqosernols73erqc3g`) branch setting; `docs/04-architecture/beta-environment-design.md` | OPEN | Needs a human decision: repoint the Render service's branch to `develop` (Render Dashboard or CLI), then confirm a fresh deploy succeeds — flagged to the human rather than silently fixed, since it's an infra/deployment config change outside this task's atomic scope. Blocks task-04 from being a meaningful check of current `agents/` code until resolved. |
