...
| 4 | The deployed `grade-ops-ai-agents` Render service's `GRADEOPS_GROQ_API_KEY` environment variable is invalid, expired, or unset. Two real runs of task-04's smoke script against the live `beta` environment (both services confirmed live and warm) failed identically at draft generation: `agents/` returned `422 {"errorCode":"MALFORMED_OUTPUT","message":"Assessment generation response could not be parsed: 401: Invalid API Key", ...}` — Groq's own API rejected the configured key. `app.agents.llm.default-provider: groq` (`agents/src/main/resources/application.yml:16`) confirms `groq` is the active default provider for this request shape, not a misconfigured provider selection. | `grade-ops-ai-agents` Render service env var `GRADEOPS_GROQ_API_KEY`; `agents/src/main/resources/application-beta.yml` | OPEN | Needs a human action: obtain/rotate a valid Groq API key and set it as `GRADEOPS_GROQ_API_KEY` on the `grade-ops-ai-agents` Render service (Dashboard → Environment), then re-run `scripts/smoke-e2e-render-beta.sh` to confirm. Flagged to the human rather than silently worked around, since this task has no ability to set Render env vars on the human's behalf without explicit instruction, and doing so would be a real production credential change. Blocks task-04's full evidence bar (a genuine generated draft from the deployed environment) until resolved. |
