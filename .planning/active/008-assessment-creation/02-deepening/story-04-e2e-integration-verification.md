...
| 4 | [Render post-deploy smoke script and evidence](story-04-e2e-integration-verification/task-04-render-post-deploy-smoke.md) | GENERATE-DOCUMENT | DONE | `scripts/smoke-e2e-render-beta.sh` using the Render CLI (`RENDER_API_KEY` auth) to confirm the latest deploy is live, then drive the same brief→generate flow against the public Render API URL; captured evidence |
...
- [x] Both smoke scripts are committed and re-runnable — not one-off manual commands lost to shell history. Both scripts confirmed with two real, passing, re-runnable live runs each: `smoke-e2e-render-beta.sh` against the deployed `beta` environment, `smoke-e2e-local.sh` (post shared-logic refactor) against the docker-compose stack once Docker came back up in this environment.
...
| 4 | Root cause of the Groq 401 confirmed to be design-doc-vs-code drift, not just an unset var. `GET /v1/services/srv-d8oqosernols73erqc3g/env-vars` (Render API, real call) showed `grade-ops-ai-agents` had exactly `INTERNAL_API_SECRET`, `AI_MODEL_NAME=gemini-2.0-flash`, `GOOGLE_AI_API_KEY=...`, `SPRING_PROFILES_ACTIVE=beta` — matching `docs/04-architecture/beta-environment-design.md` (lines 93-103, 177, 202-203) exactly. But the shipped `agents/src/main/resources/application-beta.yml` (refactored during `002-groq-genai-provider` to support both providers) expects `GRADEOPS_GEMINI_API_KEY`/`GRADEOPS_GEMINI_MODEL` (Google GenAI) and `GRADEOPS_GROQ_API_KEY`/`GRADEOPS_GROQ_MODEL` (Groq) — none of which were set. The design doc was never updated when the code's env var naming changed. | `grade-ops-ai-agents` Render service env vars; `agents/src/main/resources/application-beta.yml`; `docs/04-architecture/beta-environment-design.md` (still stale on naming) | DONE | Human added `GRADEOPS_GROQ_API_KEY`/`GRADEOPS_GROQ_MODEL` on the `grade-ops-ai-agents` Render service; it auto-restarted (`manual` deploy, live at `2026-07-16T23:47:54Z`, same commit). Re-verified with two full real runs of `scripts/smoke-e2e-render-beta.sh` — both produced genuine generated drafts. `GRADEOPS_GEMINI_API_KEY`/`GRADEOPS_GEMINI_MODEL` were not added (not needed while `default-provider: groq` stays active) and `beta-environment-design.md`'s stale `AI_MODEL_NAME`/`GOOGLE_AI_API_KEY` references were not updated — tracked as a residual below, not blocking. |
...
|---|-------------|------------|--------|
| 1 | ~~`docs/04-architecture/beta-environment-design.md` documented stale `AI_MODEL_NAME`/`GOOGLE_AI_API_KEY` env var names instead of the current code's `GRADEOPS_*`-prefixed ones.~~ **Resolved 2026-07-17**: updated the doc's YAML example, provider table, and both env var listings (`GRADEOPS_GEMINI_API_KEY`/`GRADEOPS_GEMINI_MODEL`/`GRADEOPS_GROQ_API_KEY`/`GRADEOPS_GROQ_MODEL`), and added a callout flagging the still-open `default-provider: groq` vs. the doc's "Google AI Studio is primary" framing — that specific behavioral question was deliberately left open, not silently resolved. | — | DONE |
| 2 | ~~`scripts/smoke-e2e-local.sh` was refactored in task-04 (shared logic extracted to `scripts/lib/e2e-smoke-flow.sh`) but not re-run live afterward — Docker was unavailable in this development environment.~~ **Resolved 2026-07-17**: once Docker was available, ran `scripts/smoke-e2e-local.sh` twice for real — both passed cleanly with distinct generated drafts and distinct persisted `AgentExecutionLog` rows, confirming the earlier code-review-based verification was correct. | — | DONE |
