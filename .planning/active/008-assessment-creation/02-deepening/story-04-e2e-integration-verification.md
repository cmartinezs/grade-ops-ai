...
| 2 | [Local end-to-end smoke script and evidence](story-04-e2e-integration-verification/task-02-local-e2e-smoke.md) | GENERATE-DOCUMENT | IN PROGRESS | `scripts/smoke-e2e-local.sh` driving a real brief→generate flow through the compose stack; captured evidence of a real `AssessmentDraft` + `AgentExecutionLog` produced via a genuine Groq/Gemini call |
