...
- **Interfaces / contracts:** `SecurityContextHolder.getContext().getAuthentication().getPrincipal()` returns `AuthenticatedTeacher(uid, email)` for authenticated requests. Consumed by controllers and `OwnershipVerifier` (story-07).
