...
> **Depends On:** task-02
> [← story file](../story-01-teacher-login.md)
...
- **Design notes:** Use Firebase `onAuthStateChanged` to reactively detect auth state. Show a loading skeleton during the auth check to avoid flash of protected content. Also redirect to `/verify-email` if user is authenticated but `email_verified = false` (complements story-09).
