...
- **Design notes:** Teacher scoping is enforced by `OwnershipVerifier` (story-07) — `AssessmentRepository` always filters by `teacher_firebase_uid`. Status values come from the `AssessmentStatus` enum owned by `api/` (not invented in the frontend). The response type is the canonical DTO that `web/` mirrors.
