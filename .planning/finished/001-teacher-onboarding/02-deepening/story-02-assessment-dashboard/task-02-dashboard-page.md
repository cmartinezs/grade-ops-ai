...
> **Depends On:** task-01
> [← story file](../story-02-assessment-dashboard.md)
...
- **Approach:** Next.js App Router page under the `(protected)` route group (wrapped by `AuthGuard` from story-01 task-03). Fetches `GET /assessments` via the API client. Maps `AssessmentSummaryDto[]` to a list of `AssessmentCard` components. Conditionally renders a report/log link only when `reportLink !== null`. Status and pending-approval count displayed as-is from the API (no frontend state invention). The empty-state rendering (no assessments) is handled in story-05 — this page only renders the list when data is present; it also works correctly with 0 items by rendering an empty list (no errors).
...
4. Create `web/src/app/(protected)/dashboard/page.tsx` (can be a Server Component if using Next.js server-side fetch with session token, or a Client Component using SWR/useEffect): fetches assessments, maps to `<AssessmentCard>` list, renders empty container (not empty state — that's story-05) on zero items.
...
- [x] No story creep: the task still satisfies `[CHECK-ATOMICITY]`.
