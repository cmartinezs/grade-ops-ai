...
- **Approach:** Flyway migration adding nullable columns to the existing `teacher` table (from story-06 V1). Columns: `plan_type` (VARCHAR enum: `pilot`, `free`, `paid`), `related_party` (BOOLEAN), `offer_details` (TEXT), `evidence_link` (TEXT), `flag_set_by` (VARCHAR — operator identifier), `flag_set_at` (TIMESTAMP). All nullable (existing teachers have no flag).
- **Affected files / components:**
  - `api/src/main/resources/db/migration/V2__add_pilot_flag_columns.sql` (new)
- **Interfaces / contracts:** New columns consumed by `PilotFlagRepository` / `TeacherEntity` (story-03 task-02). `related_party` maps to `RevenueEvent.related_party` semantics from `docs/05-evidence/`.
