...
> **Depends On:** —
> [← story file](../story-06-teacher-account-provisioning.md)
...
- **Interfaces / contracts:** Table `teacher(firebase_uid, name, email, created_at, updated_at)`. Consumed by `TeacherRepository` (story-06 task-03) and `TeacherEntity` JPA entity.
- **Design notes:** Pilot flag columns (`plan_type`, `related_party`, etc.) are added in story-03 task-01 as a separate migration (`V2__`). This migration is intentionally minimal. `email` uniqueness is enforced at DB level; Firebase Auth also prevents duplicate emails, but dual enforcement is safer.
