...
> **Depends On:** —
> [← story file](../story-08-teacher-self-registration.md)
...
- **Affected files / components:**
  ...
  - `api/src/main/java/com/gradeops/api/domain/teacher/TeacherRepository.java` (reuses from story-06)
  - `api/src/main/java/com/gradeops/api/domain/teacher/TeacherEntity.java` (reuses from story-06)
...
- **Design notes:** This endpoint is on the public path (no auth required). Validates the token to prevent fake registrations. Uses `saveIfAbsent` semantics — if a teacher record already exists for the UID (re-registration attempt), return 200. `emailVerified` status is not checked here — story-09 handles that gate for subsequent requests. Self-registered accounts start with `emailVerified = false`; verification is triggered client-side by story-09.
...
- [ ] No story creep: the task still satisfies `[CHECK-ATOMICITY]`.
