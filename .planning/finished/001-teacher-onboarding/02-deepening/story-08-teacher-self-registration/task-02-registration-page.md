...
- **Approach:** Next.js App Router page. Use the Firebase client SDK (`firebase/auth`) for `createUserWithEmailAndPassword`. On success, get the ID token (`user.getIdToken()`), POST it with the teacher's name to `POST /auth/register` in `api/`. On success, navigate to the email verification pending screen (story-09). Error states: `auth/email-already-in-use` → show "Email already registered" message.
- **Affected files / components:**
  - `web/src/app/register/page.tsx` (new — registration form)
  - `web/src/lib/firebase/client.ts` (new or updated — `initializeApp` with env vars from story-10 task-03)
