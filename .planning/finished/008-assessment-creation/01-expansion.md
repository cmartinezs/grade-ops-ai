...
|---|-------|------|------------|------|----------------|--------|
| 01 | agents-assessment-agent-coordination | AG | — | M | — | DONE |
| 02 | api-assessment-creation-coordination | AP | 01 | M | — | DONE |
| 03 | web-assessment-creation-coordination | WB | 02 | L | — | DONE |
| 04 | e2e-integration-verification | IN | 01, 02 | M | — | DONE |
| 05 | automated-cross-service-test-suite | IN | 01, 02, 04 | M | — | SKIPPED |
...
>
> **Release bridge update (2026-07-21):** the planning-plugin fit review report `claude-planning-with-ai:docs/plugin-review/08-gradeops-master-plan-fit.md` was applied as [R01-RELEASE-BRIDGE.md](R01-RELEASE-BRIDGE.md). That file is now the parent handoff from the R01 release document into child planning briefs for `api/`, `agents/`, `web` and `infra/docs`. It does not create child implementation tasks in this root planning.
>
> **Testing strategy update (2026-07-21):** continuing R01 or any follow-up child planning must apply `docs/master-plan/analysis/testing-strategy.md`. R01 now requires testing gates for unit/component/coverage baseline, Assessment Web-API/API-Agents contracts, isolated acceptance and minimum Compose `api-agents`/`web-api`/`full-chain` coverage when implementation starts.
>
> **UI action reachability update (2026-07-21):** every web functionality must be reachable from a visible UI action, not only by typing a URL. For R01 specifically, `/assessments/new` must be reachable from the existing "Nueva evaluacion" action on `/dashboard`, and unit/acceptance/e2e tests must cover that navigation before the intake flow is considered done.
>
> **UI Design/Data Semantics update (2026-07-21):** every UI implementation must start from the Design System and a field matrix before wireframe/mockup. For R01, `/assessments/new` must not close as all-text-input: `learningGoal` is long free text, `level` is enum/difficulty, `duration` is numeric/preset, `language` is enum/catalog, and `topic` is candidate curriculum master-data/tag/custom-controlled. If `api/` does not yet expose the needed catalog/source of truth, create child scope or record residual explicitly.
>
> **API I/O + sync/async contract update (2026-07-21):** every screen datum required for reading or writing must map to `api/`. If `api/` does not expose the needed endpoint, read model, catalog, capability, mutation or operation state, the owning child planning must implement it or record a blocking residual before UI closure. Each user action must also declare sync vs async communication; async flows must define how completion/progress/failure is observed by `web` (operation polling, SSE, WebSocket, webhook server-to-server/push notification or another explicit contract).
>
> **i18n update (2026-07-21):** R01 implementation must apply `docs/master-plan/analysis/i18n-strategy.md`. Source code, technical contracts, logs and telemetry remain in English. User-facing UI copy, safe errors, catalog labels and generated assessment drafts must respect effective locale/`outputLocale`; `language` in the intake remains programming language, not natural-language locale.
>
> **Coordination closure update (2026-07-22):** Stories 01, 02 and 03 reached `DONE` together while reconciling parent/child state for the pre-master-plan cleanup. `web/.planning/active/001-assessment-creation` resolved both of its stories (01 `SKIPPED` — scope absorbed by 02; 02 `DONE` — real end-to-end `web/`↔`api/` wiring plus a deterministic Playwright suite), which closes Story 03's Done Criteria directly, closes Story 02's checkpoint 4 (web/ integration proven), and — together with Story 04's already-`DONE` cross-service reachability proof — closes Story 01's checkpoint 3 (agent endpoint reachable from `api/` in a real, non-mocked environment). Story 05 (`automated-cross-service-test-suite`) was never atomized and closes as `SKIPPED`; its scope is deferred as Master Plan input, not open debt of this planning. See each story file for the full evidence trail.
...
|------|------|----------|-------------|
| DO | `docs/` | ☑ | Master Plan, US-010 y bridge R01 incorporan UI Design/Data Semantics, API I/O, sync/async, i18n, datos maestros y criterios de pruebas |
| WB | `web/` | ☑ | Intake form (learning goal, topic, level/difficulty, duración, lenguaje) con React Hook Form + Zod y controles semanticos DS; vista de draft editable (title, context, instructions, objectives, deliverables, constraints); acción de regeneración con adjustment notes; vista de versiones previas — **implemented in child planning `web/.planning/001-assessment-creation`, tracked here via Story 03** |
...
| `../gradeops-api` (`api/`) | `gradeops-api/story-01-assessment-creation-persistence` | [003-assessment-creation](../../../api/.planning/finished/003-assessment-creation/README.md) | child | Persists brief/draft, calls `agents/` via `agentclient`, exposes the endpoints `web/` (Story 03) consumes. Depends on the `agents/` child planning's contract. | DONE (2026-07-14) — story-01 `assessment-creation-persistence` DONE, 11/11 tasks, PR #44 merged; child planning archived to `api/.planning/finished/`. Real cross-service reachability (`api/` → `agents/`) proven by this planning's Story 04 (DONE, 2026-07-14) via a real docker-compose run and Render `beta` post-deploy smoke check, not mocks. |
| `../gradeops-web` (`web/`) | `gradeops-web/story-01-assessment-creation-ui` | [001-assessment-creation](../../../web/.planning/active/001-assessment-creation/README.md) | child | Builds the intake form, draft view/edit, regenerate action, and version history UI. Depends on `api/`'s child planning exposing real endpoints — already `DONE`, so no "wait for the contract to stabilize" risk exists here the way it did for `api/`. | DONE (2026-07-22) — `web/.planning/active/001-assessment-creation` resolved: Story 01 (`assessment-creation-ui`) `SKIPPED` 2026-07-15 (scope fully absorbed by Story 02 during atomization); Story 02 (`assessment-screens-wireframes-and-data-providers`) `DONE`, 15/15 tasks, real API wiring verified end-to-end (task-13) and covered by a deterministic Playwright e2e suite (task-15). Residual `R-POST-01` (i18n/UI-data-semantics/testing revalidation under updated Master Plan gates) carried forward as Master Plan input, not blocking this planning's closure. |
...
- **Design System y semantica de datos:** wireframes/mockups no reemplazan el diseno previo DS. Antes de implementar UI, cada campo debe tener fuente de verdad, restricciones, cardinalidad y control; datos maestros/restringidos no se vuelven `input text` libre por defecto.
- **API I/O y sync/async:** toda pantalla debe alinear datos de lectura/escritura con `api/`. Si falta contrato/API/DB/infra para que la UI trabaje con esos datos, se crea scope en el hijo correspondiente o residual bloqueante. Las acciones deben declarar sync/async; si son async, `web` debe conocer completion/progress/failure por el mecanismo acordado, no por timers locales.
- **i18n:** codigo, DTO fields, enum/error/event/metric/span codes, logs y telemetria permanecen en ingles. Copy user-facing, safe errors, catalog labels y drafts generados usan locale efectivo/`outputLocale`; las tareas deben separar `language` de programacion de locale natural.
...
| R-04 | Both child plannings' automated test suites mock the `api/`↔`agents/` network call at every layer — no test today proves the real HTTP path works, only that each side's own logic is individually correct | M | M | Story 04 closes this directly: a real docker-compose run and a real Render `beta` post-deploy smoke check, neither relying on mocks | parent owner | Mitigated (2026-07-22) — Story 04 DONE |
| R-05 | Intake UI treats restricted/master/numeric fields as arbitrary text because the current DTO shape is string-based | H | H | Child web planning must apply UI Design/Data Semantics and child api planning must create/residualize catalog or stronger-domain support when required | web/ + api/ owners | Largely mitigated (2026-07-22) — web/story-02 shipped DS-first design + field matrix (PDR-001, task-14); field matrix still flagged ⚠️ in web's `TRACEABILITY.md`, revalidation tracked in `R-POST-01` |
| R-06 | UI reads/writes or async statuses are designed without matching `api/` contracts, causing fixtures, local DTOs, timers or spinner-only completion to become hidden production behavior | H | M | Child web/api planning must map every screen datum/action to API I/O, create API scope when missing, and define sync/async completion before marking UI tasks done | web/ + api/ owners | Largely mitigated (2026-07-22) — web/story-02 mapped both screens to real `api/` endpoints end-to-end (task-06, task-12, task-13); revalidation tracked in `R-POST-01` |
| R-07 | i18n is treated as UI labels only, so draft generation, safe errors, catalog labels or telemetry language drift later | H | M | Child web/api/agents planning must apply i18n gate: effective locale, `outputLocale`, translation keys, safe errors/catalog labels, and logs/telemetry in English | web/ + api/ + agents owners | Open — not yet proven; web/story-02's evidence predates the i18n gate, tracked in `R-POST-01` |
