...
| `AssessmentConfig` | ✅ | ✅ | `@Configuration` registering the four feature beans — task-03. Gated `@ConditionalOnBean(ChatClient.Builder.class)` so `GradeOpsAgentsApplicationTest#contextLoads` still passes under the `test` Spring profile, which intentionally excludes Google GenAI's autoconfiguration. |
