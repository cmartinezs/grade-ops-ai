...
|---------------|----|---|-------|
| `GroqAssessmentGenerationAdapter` | ⚠️ | N/A | Planned in story-01 task-01 — `AssessmentGenerationPort` implementation for Groq, parallel to `001`'s `GeminiAssessmentGenerationAdapter`. Not yet implemented (atomized, not executed). |
| `spring-ai-starter-model-openai` | ⚠️ | N/A | Planned Maven dependency (story-01 task-01) — Groq is consumed via its OpenAI-compatible endpoint; Spring AI has no dedicated Groq starter. |
