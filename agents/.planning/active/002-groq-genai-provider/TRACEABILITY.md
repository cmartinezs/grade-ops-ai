...
| `AssessmentCommand.provider` / `AssessmentCommand.model` | ✅ | N/A | Implemented in story-01 task-03 — nullable per-request provider/model override, default `groq`. `model` is accepted on the contract but not yet wired to change runtime behavior (each provider's `ChatClient` uses its own configured default model; no adapter accepts a per-call model override). `provider` fully drives selection via `AssessmentGenerationPortSelector`. Cross-repo impact: `api/`'s `agentclient` child planning (`api/.planning/active/003-assessment-creation`, task-05) notified 2026-07-12, same pattern as `001`'s `previousDraft` addition. |
...
|----|-------------|---------|--------|------------------|
| R-01 | `AssessmentCommand.model` is accepted on the contract (nullable, per-request) but not actually wired to change which model a provider's `ChatClient` calls — each adapter's `ChatClient` uses its provider's own configured default model (`GRADEOPS_GEMINI_MODEL`/`GRADEOPS_GROQ_MODEL`) regardless of what `model` a request sends | Not blocking — the field exists for forward compatibility (documented in `AssessmentCommand`'s Javadoc); wiring a real per-call model override would require adapters to accept a model parameter, out of task-03's explicit Implementation Steps | OPEN | Future task, if/when per-request model override becomes an actual product requirement (related to story's own Residual #1, the model/capability registry) |
