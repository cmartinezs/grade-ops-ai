...
Crear `PasswordResetCodeCleanupJob` en el infrastructure layer: una clase plana (sin `@Component`) registrada como `@Bean` en `AuthConfig`, con un método `@Scheduled(zone = "UTC")` que invoca `CleanupPasswordResetCodesUseCase` y loguea inicio, resultado y duración sin exponer datos sensibles.
...
- **Interfaces / contracts:** Ninguna expuesta hacia fuera. Consume `CleanupPasswordResetCodesUseCase` (interfaz del port, no la implementación directa).
  > **Desviación del diseño original:** el diseño decía `CleanupPasswordResetCodesHandler`. Task-06 introdujo `CleanupPasswordResetCodesUseCase` como contrato de entrada; el scheduler inyecta la interfaz para mantener la dependencia en el puerto, no en la implementación.
- **Design notes:**
  ...
  - `@Scheduled` requiere que la clase sea un bean Spring (registrado vía `@Bean` en `AuthConfig` — no `@Component`; las guidelines permiten `@Component` solo en `@RestController`) y que `@EnableScheduling` esté activo.
...
2. Crear `PasswordResetCodeCleanupJob.java` en `auth/infrastructure/adapter/in/scheduler/`:
   ...
   ```
   Registrar en `AuthConfig` como `@Bean PasswordResetCodeCleanupJob passwordResetCodeCleanupJob(CleanupPasswordResetCodesUseCase cleanupUseCase)` — sin `@Component` per project guidelines.
...
|---|-------------|--------------|
| 1 | `run()` llama a `cleanupUseCase.execute()` exactamente una vez | `verify(cleanupUseCase).execute()` con Mockito |
| 2 | Si `execute()` lanza excepción, `run()` no propaga y loguea ERROR | `when(cleanupUseCase.execute()).thenThrow(...)` → `run()` no lanza; verificar log con `ListAppender` |
...
- [x] `PasswordResetCodeCleanupJob` existe en `auth/infrastructure/adapter/in/scheduler/`
- [x] `@Scheduled(cron = "0 0 2 * * *", zone = "UTC")` presente en el método `run()`
- [x] `@EnableScheduling` activo en la aplicación
- [x] Log INFO en inicio y fin con `deleted` y `durationMs`; log ERROR en fallo
