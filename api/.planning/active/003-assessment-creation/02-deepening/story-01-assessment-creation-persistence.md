...
| 8 | [Draft regeneration endpoint](story-01-assessment-creation-persistence/task-08-draft-regeneration-endpoint.md) | GENERATE-DOCUMENT | DONE | `POST /api/v1/assessments/{id}/draft/regenerate` |
| 9 | [Draft edit endpoint](story-01-assessment-creation-persistence/task-09-draft-edit-endpoint.md) | GENERATE-DOCUMENT | DONE | `PATCH /api/v1/assessments/{id}/draft` |
| 10 | [Retrieval endpoints + dashboard wiring](story-01-assessment-creation-persistence/task-10-retrieval-and-dashboard-wiring.md) | GENERATE-DOCUMENT | DONE | `GET .../draft`, `GET .../draft/versions`, real `GET /api/v1/assessments` data |
| 11 | [End-to-end integration tests](story-01-assessment-creation-persistence/task-11-integration-tests.md) | GENERATE-DOCUMENT | IN PROGRESS | `AssessmentCreationFlowIntegrationTest.java` |
...
| 3 | Noted in task-11's human code review (2026-07-14): `@DataJpaTest` wraps every test method in a framework-managed transaction (rolled back at teardown). `DraftGenerationCoordinator`'s `TransactionTemplate` uses default `REQUIRED` propagation, so inside these tests it joins that ambient transaction instead of proving the "no DB transaction is open while `assessmentAgentClient.generate(...)` runs" invariant the class's own design comment describes. This limitation is inherited from task-07/08/09's existing `@DataJpaTest`-based integration tests (`GenerateAssessmentDraftHandlerIntegrationTest`, `RegenerateAssessmentDraftHandlerIntegrationTest`, `UpdateAssessmentDraftHandlerIntegrationTest`) and this task's `AssessmentCreationFlowIntegrationTest` alike — none of them actually exercise a transaction-free window. Not blocking: the mocked agent call is synchronous with no observable side effect from running inside vs. outside a transaction in these tests, and no correctness bug is implied. | Future task/planning, if this invariant ever needs a dedicated proof — e.g. a `@SpringBootTest` (no test-managed transaction) asserting no connection is checked out from the pool during the agent call | OPEN |
