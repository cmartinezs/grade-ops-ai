...
|---------------|----|----|---|-------|
| `Assessment` (real, replaces stub) | ✅ | N/A | ✅ | Aggregate root — task-01 DONE. Replaces `StubAssessmentPersistenceAdapter`, whose own comment predicted this exact epic. Stub replacement fully completed in task-10: `findAllByTeacherId` now returns real dashboard data (title from current draft or brief topic) instead of the empty-list-preserving placeholder task-01 intentionally left in place. |
...
| `POST /api/v1/assessments/{id}/draft/regenerate` | ✅ | N/A | ✅ | task-08 DONE — regeneration, shares the "call agents/, persist log+draft" flow with task-07's endpoint via new `DraftGenerationCoordinator` (extracted from `GenerateAssessmentDraftHandler`, required updating that already-merged class's constructor). No prior draft → `NoPriorDraftException` → 422. Adjustment notes rendered via a private `toPromptSummary()` on the handler, sent as `previousDraft`/`previousDraftId` on the outgoing `AssessmentCommand`. |
| `PATCH /api/v1/assessments/{id}/draft` | ✅ | N/A | ✅ | task-09 DONE — in-place edit via `AssessmentDraft.applyEdit(...)` (new domain object, same id/version/previousVersionId, null params keep current value) + `save()` with the existing id (JPA upsert-by-id → `UPDATE`, not `INSERT`). No `AgentExecutionLog`, no `agents/` call. Requires a prior draft (`NoPriorDraftException`, reused from task-08) → 422. |
| `GET /api/v1/assessments/{id}/draft`, `.../versions` | ✅ | N/A | ✅ | task-10 DONE. Both ownership-checked (404 on cross-teacher access, matching `OwnershipVerifier`). Current-draft 404 is a plain "no resource yet" (distinct from task-08/09's `NoPriorDraftException` 422, which is a write-operation precondition). Versions endpoint returns an empty list, not 404, when no draft exists yet. `GET /api/v1/assessments` also completed here: `AssessmentPersistenceAdapter.findAllByTeacherId` now runs one native query (Postgres `LEFT JOIN LATERAL`, no N+1) joining assessments with their brief (topic fallback) and highest-`version_number` draft (real title). |
