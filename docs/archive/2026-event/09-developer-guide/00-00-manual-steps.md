...
Overview
Create GCP account (free trial — $300 credit)

Claim hackathon credits (if available)

Create GCP project

Link billing account with credits to project

Enable Firebase on the project — one-time manual step

Register web app (Terraform or manual)

Get Firebase config keys → web/.env.local

terraform apply — provisions everything else

Deploy API and web to Cloud Run
...
If you already have a GCP account with a billing account set up, skip to Step 1.

You need a Google Account (any Gmail or Google Workspace email) before you can create a GCP account.

0.1 — Prerequisites
A Google Account (gmail or Google Workspace). If you don't have one, create it at accounts.google.com.
...
Go to cloud.google.com/free and click "Get started for free" (or "Start Free").
Sign in with your Google Account.
Select your country and read/accept the Terms of Service.
Choose your account type:
Individual — for personal projects and hackathons
Business — if you have a company (not needed for the hackathon)
Enter your name, address, and payment method (credit/debit card).
...
Log in to Devpost with the account used for your team's submission.
Navigate to the sponsor resources or "Prizes" section.
Find the Google Cloud credit offer — it will be a coupon code or a redemption link (e.g., g.co/cloud/credits/...).
Open the redemption link while signed in to the Google account you will use for the GCP project.
Create a new Billing Account (do not attach the credits to an existing personal or company billing account — keep hackathon costs isolated).
The credits will appear under Billing → Credits in the Google Cloud Console within a few minutes.
...
Project name: gradeops-ai-demo (or your preferred name)
Organization: leave as-is (personal account = no organization)
Click Create
Note the Project ID — this is what goes into variables.tf as project_id
Attach the billing account:

In the Cloud Console, go to Billing → Manage billing accounts
Select the billing account you created in Step 1 (the one with credits)
Go to My Projects tab → find your project → Change billing → select the credits account
...
In the dropdown, select your existing GCP project (gradeops-ai-demo) — do NOT create a new project
...
You do NOT need to enable Google Analytics for the hackathon MVP
Click Continue → Firebase finishes linking to your GCP project
Verify it worked:
...
Enter an app nickname (e.g., gradeops-ai-web). This is for internal identification only.
Click Register app
The console displays a Firebase configuration object with all the keys you need:
...
Copy these values — they go into web/.env.local (see Step 7 for the format).
...
Cloud Storage
Artifact Registry
These are deployed manually with gcloud commands for the hackathon demo — see 09-deployment-guide.md.
...
Select your web app by nickname
Click Config in the Firebase SDK snippet pane
...
Google Cloud Console → Billing → Reports — filter by this project
Set up a budget alert at $50 to avoid unexpected charges if credits run out
Note the credit balance: Billing → Credits — shows remaining balance
...
See ../05-evidence/ for the cost evidence template.
