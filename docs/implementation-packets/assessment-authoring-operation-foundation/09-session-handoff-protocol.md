...
- **When each session pushes:** immediately after its own local verification (full test suite for that workspace) is green and its `*-HANDOFF.md` is written — do not batch multiple sessions' work into one push.
...
Each local packet's `HANDOFF.md` (filled in by that session as `API-HANDOFF.md`/`AGENTS-HANDOFF.md`/`WEB-HANDOFF.md` once work is done — the template lives in the packet, the filled version is what gets committed) must contain:
