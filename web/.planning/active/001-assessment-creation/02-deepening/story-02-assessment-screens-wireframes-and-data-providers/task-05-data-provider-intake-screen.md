...
- **Correlation / trace context:** A client-generated correlation id (UUID v4) is created once at the start of `submitAssessmentBrief` and passed as a bound field to a Pino child logger, so both `createAssessmentBrief` and `generateAssessmentDraft` log under the same `correlationId` and a failure between the two steps is traceable as one logical operation. Not propagated as an HTTP header to the backend in this task (per `.planning/LOGGING.md`'s note — no documented backend-side correlation header contract exists yet).
